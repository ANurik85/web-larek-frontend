...
```
interface IOrder extends IFormContacts, IFormAddress {
  items: string[]
}
```
...
- `items` Массив, который хранит товары в корзине.
- `title` HTML-элемент, который представляет собой заголовок товара.
- `addButton` HTML-элемент кнопки для добавления товара в корзину.
...
- `add(id: string)` - Добавляет товар в корзину, увеличивая его количество на единицу. Если товара нет в корзине, он создается с количеством 0 и затем увеличивается на единицу.

- `remove(id: string)` - Удаляет товар из корзины, уменьшая его количество на единицу. Если количество товара достигает 0, он удаляется из корзины.

- `getItems()` - Возвращает массив объектов, содержащих идентификатор и количество каждого товара в корзине.

- `_changed()` - Отправляет событие basket:change с массивом идентификаторов товаров в корзине, когда корзина изменяется. Этот метод вызывается автоматически после добавления или удаления товара из корзины.
...
Класс `CatalogModel` представляет собой модель каталога товаров. Он реализует интерфейс `ICatalogModel`. Этот класс предназначен для организации и управления списком товаров в каталоге. Он позволяет устанавливать и получать список товаров, а также осуществлять поиск товара по его идентификатору.

Методы и поля:

- `items: IProduct[]` - Массив, хранящий список товаров в каталоге.
...
- Поля: `_phone` и `_email`  используются для хранения ссылок на соответствующие элементы формы.
...
- `counter: number` - элемент для показа количество товара в корзину.
- `locked: boolean` - свойство для блокировки прокрутки страницу.
...
- `order:change` - событие обрабатывает изменения полей заказа.
...
- `ui:basket-add` - увеличить количество выбранного товара в корзину
- `ui:basket-remove` - удалить выбранное товара из корзину
